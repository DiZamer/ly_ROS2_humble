...
def process_image(filename):
    ...
    new_image_name = os.path.splitext(filename)[0] + '_yolo' + os.path.splitext(filename)[1]
    new_image_path = os.path.join(image_folder_path, new_image_name)

    cv2.imwrite(new_image_path, image)

    # Сохранение данных в текстовый файл
    text_file_name = os.path.splitext(filename)[0] + '_data.txt'
    text_file_path = os.path.join(image_folder_path, text_file_name)
